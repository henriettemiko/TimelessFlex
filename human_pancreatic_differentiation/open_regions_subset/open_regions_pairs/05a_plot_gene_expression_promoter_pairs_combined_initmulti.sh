...
##########
#name:          05a_plot_gene_expression_promoter_pairs_combined_initmulti.sh
#description:   plots gene expression for promoters pairs combined initmulti
...
#regions are col2-12
cut -f2-7,9,11,56 allCountsNorm_${NUM_CLUSTER_COMBINED_INITMULTI}classes.txt > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes1.bed

cut -f29-34,36,38,56 allCountsNorm_${NUM_CLUSTER_COMBINED_INITMULTI}classes.txt \
    > regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes2.bed 
...
awk 'OFS="\t" {if ($7!="." && $8==".") print $0}' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes1.bed > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_uni1.txt

awk 'OFS="\t" {if ($7!="." && $8==".") print $0}' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes2.bed > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_uni2.txt
...
print $1,$2,$3,$4,$5,"+",$7,".",$9"\n"$1,$2,$3,$4,$5,"-",$8,".",$9}' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes1.bed > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_bidir1.txt

awk 'OFS="\t" {if ($7!="." && $8!=".") 
print $1,$2,$3,$4,$5,"+",$7,".",$9"\n"$1,$2,$3,$4,$5,"-",$8,".",$9}' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes2.bed > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_bidir2.txt

cat regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_uni1.txt \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_bidir1.txt | \
    sort -k1,1 -k2,2n > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom1.txt

cat regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_uni2.txt \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_bidir2.txt | \
    sort -k1,1 -k2,2n > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2.txt
...
cut -f 7,9 regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom1.txt > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_1.txt
cut -f 7,9 regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2.txt > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_2.txt
...
awk '!seen[$0]++' regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_1.txt \
    > regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup1.txt
awk '!seen[$0]++' regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_2.txt \
    > regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup2.txt
...
if (c[i] == 1) for (j = 1; j <= c[i]; j++) print l[i,j] } }' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup1.txt | \
    sort > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign1.txt

awk 'BEGIN { FS="\t" } { c[$1]++; l[$1,c[$1]]=$0 } END { for (i in c) { 
if (c[i] == 1) for (j = 1; j <= c[i]; j++) print l[i,j] } }' \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup2.txt | \
    sort > \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign2.txt
...
join -t $'\t' all_join.txt \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign1.txt \
    > join_${NUM_CLUSTER_COMBINED_INITMULTI}assignments1.txt

join -t $'\t' all_join.txt \
    regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign2.txt \
    > join_${NUM_CLUSTER_COMBINED_INITMULTI}assignments2.txt
...
plot_gene_expression_cluster_prom-prom.r \
    join_${NUM_CLUSTER_COMBINED_INITMULTI}assignments1.txt \
    join_${NUM_CLUSTER_COMBINED_INITMULTI}assignments2.txt ${NUM_TIME_POINTS} \
    ${NUM_REPLICATES} ${NUM_CLUSTER_COMBINED_INITMULTI} \
    $CUR_DIR_COMBINED_INITMULTI

#this is all genes that are unambigously assigned to one clusters, 
#col1 gene id, col2 assignment
cp regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign1.txt \
    genes_assignment1.txt
cp regions_${NUM_CLUSTER_COMBINED_INITMULTI}classes_allprom2_nodup_sameassign2.txt \
