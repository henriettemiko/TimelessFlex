...
##########
#name:          05b_GO_analysis_pairs_multi.sh
#description:   calls GO analysis on promoter clusters from pairs multi
...
#split to cluster
for ((i=1; i<=$NUM_CLUSTER_MULTI_PROM_ENH; i++)); do echo $i; awk -v i="$i" \
...
#split to cluster
for ((i=1; i<=$NUM_CLUSTER_MULTI_PROM_PROM; i++)); do echo $i; awk -v i="$i" \
    'OFS="\t" {if ($3==i) print $0 > "genes_names_assignment_class"i".txt"}' \
    genes_names_assignment.txt; done
