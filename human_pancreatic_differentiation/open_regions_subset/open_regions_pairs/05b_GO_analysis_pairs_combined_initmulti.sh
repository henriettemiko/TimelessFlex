...
##########
#name:          05b_GO_analysis_pairs_combined_initmulti.sh
#description:   calls GO analysis on promoter clusters from pairs combined initmulti
...
#split to cluster
for ((i=1; i<=$NUM_CLUSTER_COMBINED_INITMULTI; i++)); do echo $i; awk -v i="$i" \
...
Rscript $SCRIPT_DIR/open_regions_subset/open_regions_pairs/go_analysis.r \
    $NUM_CLUSTER_COMBINED_INITMULTI \
