...
##########
#name:          plot_clusters_coherence.r
#description:   plots cluster coherence for counts subset
...
write.table(coherence,file=paste0(type, "_", numClusters, "_coherence_counts.txt"), sep="\t", quote=F, row.names=F, col.names=F)

write.table(correlation.means,file=paste0(type, "_", numClusters, "_correlations_counts.txt"), sep="\t", quote=F, row.names=F, col.names=F)


q()


pdf(paste0(type, "_clusters_", numClusters, "_coherence.pdf"), height=6, width=8)
plot(coherence, xlab="clusters", ylab="cluster coherence", ylim=c(0,1), pch=19)
axis(side=1, at=1:numClusters, labels=1:numClusters)
dev.off()

pdf(paste0(type, "_clusters_", numClusters, "_coherence_correlations.pdf"), height=6, width=8)
