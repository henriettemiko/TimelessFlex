...
##########
#name:          05b_signature_genes_promoter_split.sh
#description:   plots number of signature genes and promoter clusters split
...
join -t $'\t' <( cut -f2,3 genes_names_assignment.txt | sort -k1,1) \
    <(sort -k1,1 $SIGNATURE_GENES_DIR/FG_signature_genes.txt) > FG_genes.txt

cut -f2 FG_genes.txt | sort -n | uniq -c > FG_genes_clusters.txt
...
Rscript $SCRIPT_DIR/open_regions_split/plot_cluster_signature_genes_split.r \
