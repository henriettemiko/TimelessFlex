...
##########
#name:          06a_do_ATAC_promoter_split.sh
#description:   plots ATAC signal for clusters split
...
paste regions_${NUM_CLUSTER_PROM}_D0_cutsites.bed \
    ...
    <( cut -f 11  regions_${NUM_CLUSTER_PROM}_D8_cutsites.bed ) | \
...
for ((i=1; i<=$NUM_CLUSTER_PROM; i++)); do echo $i; \
    awk -v i="$i" -v numcluster="${NUM_CLUSTER_PROM}" 'OFS="\t" {if ($36==i) 
...
for ((i=1; i<=$NUM_CLUSTER_PROM; i++)); do echo $i;
    ...
    awk -F"D8" '{print NF-1}' \
        merged_peaks_regions_${NUM_CLUSTER_PROM}_cluster${i}.txt > \
        merged_peaks_regions_${NUM_CLUSTER_PROM}_cluster${i}_D8.txt
