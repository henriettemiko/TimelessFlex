...
##########
#name:          plot_gene_expression_cluster_split.r
#description:   plot gene expression for promoter clusters split
...
#reorder to cluster,D0,D2,D5,D7,D10 from cluster,D0,D10,D2,D5,D7
mean.fpkm.reordered = mean.fpkm[,c(1,2,4,5,6,3)]
geomav.fpkm.reordered = geomav.fpkm[,c(1,2,4,5,6,3)]



#add D8 values
mean.fpkm.reordered = cbind(mean.fpkm.reordered,fpkms[,17])
geomav.fpkm.reordered = cbind(geomav.fpkm.reordered,fpkms[,17])
...
for (i in 1:numClusters) {
    ...
    cur.scores <- geomav.fpkm.reordered[cur.items,2:7]

    boxplot(log(cur.scores+1), main=paste0("Cluster ", i, ": ", 
                                           length(cur.items), " genes"), 
            names = c("D0", "D2", "D5", "D7", "D10", "D8"), outline = F, 
...
for (i in 1:numClusters) {
    ...
    cur.scores <- mean.fpkm.reordered[cur.items,2:7]
