##########
#name:          baseline_kmeans_normfold.sh
#description:   basline k-means on normalized fold changes
...
#normalized fold-changes as input
...
fviz_cluster(data1.kmeans, data1.counts, ellipse.type = "norm")+
    theme_minimal()
ggsave(paste0(type, "_", numClusters, "_baseline_normfold_clus.pdf"), height=6, width=8)


sil <- silhouette(data1.kmeans$cluster, dist(data1.counts))
fviz_silhouette(sil)

ggsave(paste0(type, "_", numClusters, "_baseline_normfold_sil.pdf"), height=6, width=8)


write.table(data1.kmeans$cluster, sep="\t", file=paste0(type, "_", numClusters, "_baseline_normfold.txt"), quote=F, row.names=F, col.names=F)
...
l = read.table(paste0(numcluster.dir, "/", numClusters, "/", type, "_", numClusters, "_baseline_normfold.txt"))
...
pdf(paste0(type, "_clusters_", numClusters, "_baseline_normfold.pdf"), height=6, width=4)
