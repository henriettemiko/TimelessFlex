...
#choose cluster number for enhancer clusters
NUM_CLUSTER_ENH=16
...
cut -f1,2,3,4,5,6,8,10,28 allCountsNorm_${NUM_CLUSTER_ENH}classes.txt > \
...
for ((i=1; i<=$NUM_CLUSTER_ENH; i++)); do echo $i; \
    awk -v i="$i" -v numcluster="${NUM_CLUSTER_ENH}" 'OFS="\t" {if ($28==i) 
