...
disp(numTimePts);
disp(numTracks)
disp(signalGeneratorDirPromEnhInit);
...
allcov(1,1,(numClustersPromEnh+1):(numClustersPromEnh+numClustersPromProm))= ...
struct(PromProm.initModel.tan.CPD{t}).cov;
...
allnet.CPD{1} = tabular_CPD(allnet, 1, 'CPT', [((3617+3406)/(9684+9555))*struct(PromEnh.initModel.tan.CPD{1}).CPT; ((800+687)/(9684+9555))*struct(PromProm.initModel.tan.CPD{1}).CPT; ((5267+5462)/(9684+9555))*struct(EnhEnh.initModel.tan.CPD{1}).CPT]);


%check if probs sum up to 1
sumprobs=cumsum([((3617+3406)/(9684+9555))*struct(PromEnh.initModel.tan.CPD{1}).CPT;...
((800+687)/(9684+9555))*struct(PromProm.initModel.tan.CPD{1}).CPT;...
((5267+5462)/(9684+9555))*struct(EnhEnh.initModel.tan.CPD{1}).CPT]);
disp(sumprobs);
%%
...
%%%
%now cluster all init and multi pairs together with combined model
%load init and multi pairs

promEnhInit = [signalGeneratorDirPromEnhInit '/allFold_data.txt'];
disp(promEnhInit);
promPromInit = [signalGeneratorDirPromPromInit '/allFold_data.txt'];
disp(promPromInit);
enhEnhInit = [signalGeneratorDirEnhEnhInit '/allFold_data.txt'];
disp(enhEnhInit);
promEnhMulti = [signalGeneratorDirPromEnhMulti '/allFold_data.txt'];
disp(promEnhMulti);
promPromMulti = [signalGeneratorDirPromPromMulti '/allFold_data.txt'];
disp(promPromMulti);
enhEnhMulti = [signalGeneratorDirEnhEnhMulti '/allFold_data.txt'];
disp(enhEnhMulti);


pemi = importdata(promEnhInit, '\t');
ppmi = importdata(promPromInit, '\t');
eemi = importdata(enhEnhInit, '\t');

pemm = importdata(promEnhMulti, '\t');
ppmm = importdata(promPromMulti, '\t');
eemm = importdata(enhEnhMulti, '\t');


dataOrig = [ pemi; ppmi; eemi; pemm; ppmm; eemm ];
