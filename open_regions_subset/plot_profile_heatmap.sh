...
#description:   call computeMatrix and plotHeatmap from deeptools for subset
...
if [ $MARK == "ATAC" ]
    then
    FILES=($OUTPUT_DIR/ATAC/D0/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ATAC/D10/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ATAC/D2/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ATAC/D5/bedgraphs/*.bigWig)
    echo "${FILES[@]}"
else 
    FILES=($OUTPUT_DIR/ChIP/$MARK/D0/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ChIP/$MARK/D10/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ChIP/$MARK/D2/bedgraphs/*.bigWig \
    $OUTPUT_DIR/ChIP/$MARK/D5/bedgraphs/*.bigWig)
    echo "${FILES[@]}"
fi
