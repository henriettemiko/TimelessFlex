...
for t in "${TIMES[@]}"; do echo $t; \
    awk -v t="$t" -v numcluster="${NUM_CLUSTER_PROM}" \
    'OFS="\t" {len=$3-$2; print $0,$10/len > 
"regions_"numcluster"_"t"_cutsites.bed"}' \
...
for ((i=1; i<=$NUM_CLUSTER_PROM; i++)); do echo $i; \
    awk -v i="$i" -v numcluster="${NUM_CLUSTER_PROM}" \
    'OFS="\t" {if ($32==i) print $7 > 
"merged_peaks_regions_"numcluster"_cluster"i".txt"}' \
...
Rscript $SCRIPT_DIR/open_regions_subset/plot_ATAC_clusters.r \
