...
##########
#name:          05a_plot_gene_expression_promoter_pairs_multi.sh
#description:   plots gene expression (FPKM from RSEM) for promoters pairs multi
...
cut -f 7,9 regions_${NUM_CLUSTER_MULTI_PROM_ENH}classes_allprom.txt > \
    regions_${NUM_CLUSTER_MULTI_PROM_ENH}classes_allprom2.txt
...
awk '!seen[$0]++' regions_${NUM_CLUSTER_MULTI_PROM_ENH}classes_allprom2.txt > \
    regions_${NUM_CLUSTER_MULTI_PROM_ENH}classes_allprom2_nodup.txt
...
classes-${NUM_CLUSTER_MULTI_PROM_PROM}_afterEM.txt > \
...
#cut to gene id and cluster number
cut -f 7,9 regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom1.txt > \
    regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2_1.txt
cut -f 7,9 regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2.txt > \
    regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2_2.txt

#genes could be in there multiple times
#multiple lines are only taken once
#gene and class assignment must be same here to be fitting the seen
awk '!seen[$0]++' regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2_1.txt \
    > regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2_nodup1.txt
awk '!seen[$0]++' regions_${NUM_CLUSTER_MULTI_PROM_PROM}classes_allprom2_2.txt \
...
plot_gene_expression_cluster_prom-prom.r \
    join_${NUM_CLUSTER_MULTI_PROM_PROM}assignments1.txt \
    join_${NUM_CLUSTER_MULTI_PROM_PROM}assignments2.txt ${NUM_TIME_POINTS} \
    ${NUM_REPLICATES} ${NUM_CLUSTER_MULTI_PROM_PROM} \
    $NUM_CLUSTER_DIR_MULTI_PROM_PROM
